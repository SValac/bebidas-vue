...
const formatearIngredientes = () => {
  ...
  for (let i = 0; i <= 15; i++) {
    if (bebidas.receta[`strIngredient${i}`]) {
      ...
      let cantidad = 'At Taste'
        if (bebidas.receta[`strMeasure${i}`]){
              ...
              <DialogPanel class="relative transform overflow-hidden rounded-lg bg-white px-4 pt-5 pb-4 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-2xl sm:p-6" >
                <div>
                  <div class="mt-3">
                    ...
                    <p class="text-lg">{{ bebidas.receta.strInstructions }}</p>
                ...
                <div class="mt-5 sm:mt-6 flex justify-between gap-4">
                    <button
                        type="button"
                        class="bg-gray-500 p-3 w-full text-white uppercase font-extrabold text-lg rounded shadow hover:bg-gray-600"
                        @click="modal.handleClickModal"
                    >Cerrar</button>
                    <button
                        type="button"
                        class="bg-orange-500 p-3 w-full text-white uppercase font-extrabold text-lg rounded shadow hover:bg-orange-600"
                        @click="favoritos.handleClickFavoritos"
                    >{{ modal.textoBoton }}</button>
                </div> 
              </DialogPanel>
            </TransitionChild>
          </div>
